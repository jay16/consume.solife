...
(function() {
  window.Tags = {
    cancelEdit: function(tag_id) {
      var $dom_tag;
      $dom_tag = $("#tag_" + tag_id);
      $dom_tag.find(".edit-form").remove();
      return $dom_tag.find(".show-form").css("display", "block");
    },
    initTagIt: function(target, tags, control, show_pane) {
      var $control, $eventTags, $show_pane, addEvent, sampleTags;
      $eventTags = $(target);
      sampleTags = tags;
      $control = $(control);
      $show_pane = $(show_pane);
      addEvent = function(text) {
        var tags_list;
        tags_list = [];
        $eventTags.find(".tagit-choice:not(.removed)").each(function() {
          return tags_list.push($(this).find(".tagit-label:first").text());
        });
        if (show_pane.length) {
          $show_pane.html(text + "<br>" + tags_list.join(","));
        }
        $control.val(tags_list.join(","));
      };
      return $eventTags.tagit({
...
(function() {
  window.Records = {
    multiFileUpload: function(input_file, show_pane) {
      var multi_selector;
      multi_selector = new MultiSelector(document.getElementById(show_pane), 30);
      return multi_selector.addElement(document.getElementById(input_file));
    }
  };
