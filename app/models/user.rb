...
class User < ActiveRecord::Base
  ...
  def group_members(whether_include_self = true)
    members = group_users + group_follows
    members.push(self) if whether_include_self
    members.uniq
  end

  def group_member_records(whether_include_self = true)
    ids = group_members(whether_include_self).map(&:id)
    ::Record.where("user_id in (?)", ids)
  end
